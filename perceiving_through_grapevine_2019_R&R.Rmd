...
```{r}
consensus_sam <- lmer(p2 ~ p1_norm + p1_dist + eval +
                         (1 + p1_norm + p1_dist + eval | sess_id/triad_id),
                       data = study1_profiles)

summary(consensus_sam)
```
...
```{r}
consensus_sam_t_ext <- lmer(p2 ~ p1_norm + p1_dist + eval + t_ext_cent +
                               t_ext_cent:p1_norm + t_ext_cent:p1_dist + t_ext_cent:eval +
                               (1 + p1_norm + p1_dist + t_ext_cent +
                                  t_ext_cent:p1_norm + t_ext_cent:p1_dist + t_ext_cent:eval | 
                                  sess_id/triad_id),
                             data = study1_profiles)
summary(consensus_sam_t_ext)
```
...
```{r}
consensus_sam_p1_ext <- lmer(p2 ~ p1_norm + p1_dist + eval + p1_ext_cent +
                               p1_ext_cent:p1_norm + p1_ext_cent:p1_dist + p1_ext_cent:eval +
                               (1 + p1_norm + p1_dist + p1_ext_cent +
                                  p1_ext_cent:p1_norm + p1_ext_cent:p1_dist + p1_ext_cent:eval | 
                                  sess_id/triad_id),
                             data = study1_profiles)
summary(consensus_sam_p1_ext)
```
...
```{r}
consensus_sam_p2_ext <- lmer(p2 ~ p1_norm + p1_dist + eval + p2_ext_cent +
                               p2_ext_cent:p1_norm + p2_ext_cent:p1_dist + p2_ext_cent:eval +
                               (1 + p1_norm + p1_dist + p2_ext_cent +
                                  p2_ext_cent:p1_norm + p2_ext_cent:p1_dist + p2_ext_cent:eval | 
                                  sess_id/triad_id),
                             data = study1_profiles)
summary(consensus_sam_p2_ext)
```
...
```{r}
accuracy_sam_p2_ext <- lmer(p2 ~ self_norm + self_dist + eval + p2_ext_cent +
...
consensus_sam_distress <- lmer(p2 ~ p1_norm + p1_dist + eval + distress_cent +
                               ...
                               (1 + p1_norm + p1_dist + distress_cent +
                                  ...
                                  sess_id/triad_id),
                             data = study1_profiles)
summary(consensus_sam_distress)
```
...
consensus_sam_empathic <- lmer(p2 ~ p1_norm + p1_dist + eval + empathic_cent +
                               ...
                               (1 + p1_norm + p1_dist + empathic_cent +
                                  ...
                                  sess_id/triad_id),
                             data = study1_profiles)
summary(consensus_sam_empathic)
```
...
```{r}
consensus_sam_fantasy <- lmer(p2 ~ p1_norm + p1_dist + eval + fantasy_cent +
                               fantasy_cent:p1_norm + fantasy_cent:p1_dist + fantasy_cent:eval +
                               (1 + p1_norm + p1_dist + fantasy_cent +
                                  fantasy_cent:p1_norm + fantasy_cent:p1_dist + fantasy_cent:eval | 
                                  sess_id/triad_id),
                             data = study1_profiles)
summary(consensus_sam_fantasy)
```
...
```{r}
consensus_sam_perspective <- lmer(p2 ~ p1_norm + p1_dist + eval + perspective_cent +
                               perspective_cent:p1_norm + perspective_cent:p1_dist + perspective_cent:eval +
                               (1 + p1_norm + p1_dist + perspective_cent +
                                  perspective_cent:p1_norm + perspective_cent:p1_dist + perspective_cent:eval | 
                                  sess_id/triad_id),
                             data = study1_profiles)
summary(consensus_sam_perspective)
```
...
```{r}
accuracy_sam_distress <- lmer(p2 ~ self_norm + self_dist + eval + distress_cent +
                               distress_cent:self_norm + distress_cent:self_dist + distress_cent:eval +
                               (1 + self_norm + self_dist + distress_cent +
                                  distress_cent:self_norm + distress_cent:self_dist + distress_cent:eval | 
                                  sess_id/triad_id),
                             data = study1_profiles)
summary(accuracy_sam_distress)
```
...
```{r}
accuracy_sam_empathic <- lmer(p2 ~ self_norm + self_dist + eval + empathic_cent +
                               empathic_cent:self_norm + empathic_cent:self_dist + empathic_cent:eval +
                               (1 + self_norm + self_dist + empathic_cent +
                                  empathic_cent:self_norm + empathic_cent:self_dist + empathic_cent:eval | 
                                  sess_id/triad_id),
                             data = study1_profiles)
summary(accuracy_sam_empathic)
```
...
```{r}
accuracy_sam_fantasy <- lmer(p2 ~ self_norm + self_dist + eval + fantasy_cent +
                               fantasy_cent:self_norm + fantasy_cent:self_dist + fantasy_cent:eval +
                               (1 + self_norm + self_dist + fantasy_cent +
                                  fantasy_cent:self_norm + fantasy_cent:self_dist + fantasy_cent:eval | 
                                  sess_id/triad_id),
                             data = study1_profiles)
summary(accuracy_sam_fantasy)
```
...
```{r}
accuracy_sam_perspective <- lmer(p2 ~ self_norm + self_dist + eval + perspective_cent +
                               perspective_cent:self_norm + perspective_cent:self_dist + perspective_cent:eval +
                               (1 + self_norm + self_dist + perspective_cent +
                                  perspective_cent:self_norm + perspective_cent:self_dist + perspective_cent:eval | 
                                  sess_id/triad_id),
                             data = study1_profiles)
summary(accuracy_sam_perspective)
```
