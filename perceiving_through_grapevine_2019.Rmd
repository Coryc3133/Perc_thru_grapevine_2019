...
I'm going to use the `scoreItems()` function from the `psych` package. This both reverse scores items and calculates scale scores. It also provide some information about the scores, but that isn't particularly helpful here (since we're collapsing across rating type for the sake of calculating scores).
...
Using the `scoreItems()` function from the psych package. For this, you provide a list of the scales with a key (basically, quoted variable names with a '-' for items that need to be reverse scored). So first I'll make that key, and then I'll use the key to the score the items into a temporary object called scale_scores_nometa. Then, Ill save out just the `scores` object from the `scale_scores_nometa` object (the object also contains some information about reliability, like alpha; but this is calculated across self-, P1-, and P2-, so not very useful here). At the end of this script, we split the data and use the same scoreItems function to assess scale reliability.

For now, first make the key:
```{r make_keys_nometa}
# Make keys
key_list_big6 <- list(extraversion = c("bfi_1", "bfi_6", "-bfi_11", "-bfi_16", 
...
Next, we actually 'make the keys' with the `make.keys() function` and then pass that along to the scoreItems function. Since we did not pre-register any imputation for missing items, we'll set `impute= "none"`, which will have it take the mean across available items.

```{r score_items_nometa}
# score by 'making keys' and using those to score the items (with scoreItems)
scale_scores_nometa <- 
  make.keys(main_qs_wide_nometa, key_list_nometa) %>%
  scoreItems(main_qs_wide_nometa, impute = "none")
```

Save out just the scores from the list; going to recycle the `scale_scores_nometa` name, since we don't need that object elsewise.
```{r save_scores_nometa}
scale_scores_nometa <- scale_scores_nometa$scores
```
...
## Study 2 & Integrated Individual Difference Moderator Data Prep
Next up are the individual difference moderators. Like in Study 1, these analyses require first mean-centering predictors and creating interaction terms, which the next chunk of code does. 
```{r prep_data_integrated}
integrated <-
integrated %>%
  # First, mean center the predictors, 
  # since this is a continuous by continuous interaction.
